    ...
    >
      ...
      >
        <button
          type="button"
          onClick={onClose}
          aria-label="إغلاق تفاصيل صانع المحتوى"
        ...
        <div className="overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-brand-border">
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start gap-6 mb-8 text-center md:text-right">
            <div className="relative">
              <OptimizedImage
                ...
                seed={creator.name}
                alt={typeof creator.name === 'object' ? getLocalizedItem(creator, 'name', language) : creator.name}
                width="128"
                height="128"
              ...
              {creator.verified && (
                <div className="absolute bottom-1 right-1 bg-blue-500 rounded-full p-1 border-2 border-white shadow-sm text-white">
                  <BadgeCheck className="w-5 h-5 md:w-6 md:h-6" aria-hidden="true" />
                </div>
              )}
            </div>

            <div className="flex-1 space-y-2">
              <h2 id="creator-modal-title" className="text-3xl font-black text-brand-brown flex items-center justify-center md:justify-start gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div className="space-y-3">
              ...
              <a
                href={safeHref(creator.youtube_url || creator.social_links?.youtube)}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-between p-3 bg-white border border-brand-border rounded-2xl hover:bg-brand-cream transition-colors group shadow-sm"
              >
              ...
              <a
                href={safeHref(creator.instagram_url || creator.social_links?.instagram)}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-between p-3 bg-white border border-brand-border rounded-2xl hover:bg-brand-cream transition-colors group shadow-sm"
              >
              ...
              <a
                href={safeHref(creator.tiktok_url || creator.social_links?.tiktok)}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-between p-3 bg-white border border-brand-border rounded-2xl hover:bg-brand-cream transition-colors group shadow-sm"
              >
